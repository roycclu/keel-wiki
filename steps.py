...
def find_citation_needed(
    first_letter: str | None = None, limit: int = 15
) -> list[TargetPage]:
...
CITATION_TEMPLATE_NAMES = {"citation needed", "cn"}


def parser_extract_citation_targets(
    target_page: TargetPage, context_before_len: int = 200, context_after_len: int = 10
) -> list[CitationTarget]:
    ...
    for index, template in enumerate(wikicode.filter_templates(recursive=True)):
        ...
        wikicode.replace(marker, template)

        marker_position = marked_wikitext.index(marker)
        context_start_position = max(0, marker_position - context_before_len)
        context_end_position = min(
            len(marked_wikitext), marker_position + len(marker) + context_after_len
        )
        context = (
            marked_wikitext[context_start_position:marker_position]
            + marked_wikitext[marker_position + len(marker) : context_end_position]
        )
...
def pick_one_probable_citation_target(
...
def search_web_for_citation_support(
    citation_target: CitationTarget, limit: int = 10
...
def judge_support_from_sources_by_llm(
    *, citation_target: CitationTarget, web_hits: list[WebSearchEvidence]
) -> DecisionCitationSupport:
...
def prepare_citation_edit(
    target_page: TargetPage, citation_target: CitationTarget, web_hit: WebSearchEvidence
) -> PreparedCitationEdit:
    ...
    new_wikitext = citation_target.marked_wikitext.replace(
        citation_target.marker, citation, 1
    )
...
def submit_with_citation(
    target_page: TargetPage, citation_edit: PreparedCitationEdit
) -> CitationSubmissionResult:
    ...
    if page.latest_revision_id != target_page.base_revid:
        raise RuntimeError("Wikipedia page changed after it was retrieved")
    ...
    return CitationSubmissionResult(
        ...
        revision_url=page.permalink(oldid=revision_id, with_protocol=True),
    )
